...
public class Trie<Value>
{
    ...
    private Node Insert(Node node, string key, Value val, int position)
    {
        if(node == null)
        {
            node = new Node();
        ...
        node.next[currentChar] = this.Insert(nextNode, key, val, position + 1);
